...
- **輸出**: `cover.out`（`go tool cover -html cover.out -o cover.html` 可產生 HTML 報告）
...
### Controller Manager 端點

- 健康檢查：`curl http://localhost:8081/healthz`（readyz 同 port）
...
`hack/kind.sh` 會提示以下系統參數，若不符合建議值，大量 pod 場景可能異常：

```bash
sudo sysctl -w kernel.keys.maxkeys=2000 fs.file-max=10000000
sudo sysctl -w fs.inotify.max_user_instances=65535 fs.inotify.max_user_watches=1048576
```

### HTTP/2 CVE

Controller Manager 預設關閉 HTTP/2（`--enable-http2=false`），規避 Stream Cancellation CVE（GHSA-qppj-fm5r-hxr3）。除非明確需要，不要開啟此選項。
